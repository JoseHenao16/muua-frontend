        ...
        <div class="pieza-col-img">
          <div class="pieza-img-frame">
            <div class="pieza-img-lateral">
              {{ pieza.denominacion }} / {{ pieza.zonaArqueologica }} /
              {{ pieza.departamento }}, {{ pieza.pais }} /
              {{ pieza.cronologia }} /
...
import { getPieza, getImagenUrl, generarReporte, generarReportePdf } from 'src/services/api'
...
const imagenUrl = computed(() =>
  pieza.value ? getImagenUrl(pieza.value.numeroRegistro) : ''
)
...
async function cargarPieza() {
  ...
  error.value = null
  try {
    pieza.value = await getPieza(route.params.codigo)
  } catch (e) {
...
.campo__label--gold { color: #C9A84C; }
...
.btn-reporte:hover { background: #b8963e; border-color: #b8963e; }
.btn-reporte--outline {
  background: none;
...
.btn-reporte--outline:hover { border-color: #C9A84C; color: #C9A84C; }
...
.skeleton-caption { height: 44px; background: #1a1810; margin-top: 1px; }
.skeleton-line {
  height: 12px;
  border-radius: 2px;
  background: linear-gradient(90deg, #1a1810 25%, #231f14 50%, #1a1810 75%);
...
.skeleton-line--title { width: 80%; height: 32px; }
...
.pieza-back-link:hover { border-color: #C9A84C; }

/* ── Responsive ── */
@media (max-width: 900px) {
  .pieza-header,
  .pieza-layout { padding-left: 1.5rem; padding-right: 1.5rem; }
  .pieza-layout { grid-template-columns: 1fr; gap: 2rem; }
  .pieza-img-lateral { display: none; }
}
</style>
